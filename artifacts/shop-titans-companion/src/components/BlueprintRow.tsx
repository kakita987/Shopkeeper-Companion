...
interface BlueprintRowProps {
  blueprint: Blueprint;
  /** Called when the row is tapped to open the detail panel. */
  onSelect?: () => void;
}
 ...
 * Tapping the row fires onSelect to open the detail panel.
 */
export function BlueprintRow({ blueprint, onSelect }: BlueprintRowProps) {
  ...
  return (
    <button
      onClick={onSelect}
      className="w-full flex items-center gap-3 pl-14 pr-4 py-2.5 border-b border-border/60 last:border-b-0 hover:bg-muted/40 active:bg-muted/70 transition-colors text-left focus:outline-none"
      data-testid={`blueprint-row-${blueprint.id}`}
    >
      {/* Tier badge */}
      <span className={`text-xs font-semibold px-1.5 py-0.5 rounded shrink-0 ${getTierColor(blueprint.tier)}`}>
        T{blueprint.tier}
      </span>
