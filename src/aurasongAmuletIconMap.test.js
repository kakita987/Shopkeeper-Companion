...
} from './assets/aurasongAmuletIconMap.js'
...
test('buildAurasongAmuletIconMapFromImport derives names and tiers from importer rows', () => {
  ...
  assert.match(mapRows[0].typeIconPath, /\/assets\/Types\/accessory_amulet_type\.png$/)
  assert.match(mapRows[1].typeIconPath, /\/assets\/Types\/weapon_aurasong_type\.png$/)
  assert.match(mapRows[0].itemIconRelativePath, /\/assets\/Items\/Amulets\//)
  assert.match(mapRows[1].itemIconRelativePath, /\/assets\/Items\/Aurasongs\//)
})
...
test('buildAurasongAmuletIconMapFromImport falls back to unique type+tier icon when name differs', () => {
  ...
  assert.match(mapRows[0].itemIconRelativePath, /\/assets\/Items\/Amulets\/accessory_amulet_t1_ironamulet\.png$/)
})
...
test('buildAurasongAmuletIconMapFromImport infers target type from Accessory rows', () => {
  ...
  assert.equal(byKey['memento::1']?.type, 'Amulet')
...
test('toAurasongAmuletIconMapCsv serializes deterministic headers and rows', () => {
  const csv = toAurasongAmuletIconMapCsv([
    {
      ...
      group: 'Accessories',
      type: 'Amulet',
      typeIconPath: '/assets/Types/accessory_amulet_type.png',
      itemIconRelativePath: './assets/Items/Amulets/accessory_amulet_t5_luckymedallion.png',
  ...
  assert.equal(lines[1], 'Lucky Medallion,5,Accessories,Amulet,/assets/Types/accessory_amulet_type.png,./assets/Items/Amulets/accessory_amulet_t5_luckymedallion.png,luckymedallion::5')
})
...
test('applyAurasongAmuletTypeMap forces mapped Aurasong/Amulet types by name+tier key', () => {
  ...
  assert.match(corrected[0].iconMapping?.itemIconRelativePath || '', /\/assets\/Items\/Amulets\//)
  assert.match(corrected[1].iconMapping?.itemIconRelativePath || '', /\/assets\/Items\/Aurasongs\//)
})